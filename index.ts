...
const compileJavaToJs = async (javaCode: string) => {
    const java = require('java');
    const execAsync = util.promisify(exec);
    const tempDir = await fs.promises.mkdtemp(path.join('/tmp', 'java-'));
...
const makeFunctionJava = async (code: string) => {
    const jsFilePath = await compileJavaToJs(code);
...
app.post('/call', async (req, res) => {
  try {
    ...
    const fn = await makeFunctionJava(code);
